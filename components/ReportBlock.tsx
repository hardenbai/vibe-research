...
function TextModule({ chapterId, subChapterId, module }: Props) {
  ...
  const handleExpand = async () => {
    ...
    try {
      ...
      if (content) { updateReport(chapterId, subChapterId, module.id, { content: report.content ? report.content + '\n\n' + content : content }); setViewpoint('') }
    } catch (err: unknown) { setError(err instanceof Error ? err.message : 'AI 生成失败') } finally { setLoading(false) }
  }

  const handleDraftFromSources = async () => {
    const apiKey = aiSettings.apiKeys[aiSettings.providerId]
    if (!apiKey) { setError('请先在 AI 设置中填入 API Key'); return }
    setError(''); setLoading(true)
    try {
      const provider = PROVIDER_CONFIGS.find(p => p.id === aiSettings.providerId)
      const content = await callAI(aiSettings.providerId, apiKey, aiSettings.modelId, provider?.baseUrl, DRAFT_PROMPT,
        buildDraftPrompt(draft.sources ?? [], report.content))
      if (content) updateReport(chapterId, subChapterId, module.id, { content: report.content ? report.content + '\n\n' + content : content })
    } catch (err: unknown) { setError(err instanceof Error ? err.message : 'AI 生成失败') } finally { setLoading(false) }
  }
  ...
  return (
    <div className="flex flex-col gap-2 p-3 bg-gray-800 rounded-lg border border-gray-700 min-h-[120px]">
      ...
      {hasSources && (
        <button onClick={handleDraftFromSources} disabled={loading}
          className="w-full py-2 bg-green-700/30 hover:bg-green-700/50 disabled:bg-gray-700 border border-green-800 hover:border-green-600 text-green-400 disabled:text-gray-500 text-xs rounded-lg transition-colors flex items-center justify-center gap-1.5">
          <span>📝</span><span>{loading ? '生成中...' : '根据底稿生成内容'}</span>
        </button>
      )}
      ...
      <div className="flex gap-2">
        ...
        <button onClick={handleExpand} disabled={loading || !viewpoint.trim()}
