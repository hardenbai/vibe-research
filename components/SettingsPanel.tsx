  ...
  return (
    <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div key={m.id + i} className="group flex items-center gap-2 px-3 py-2 rounded-lg"
            ...
            {editIdx === i ? (
              <>
                ...
                <button onClick={commit} style={{ color: 'var(--green)', fontSize: 14 }}>✓</button>
                <button onClick={() => setEditIdx(null)} style={{ color: 'var(--t4)', fontSize: 12 }}>✕</button>
              </>
            ) : (
              <>
                ...
                <button onClick={() => { setEditIdx(i); setEditId(m.id); setEditLabel(m.label) }} className="opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontSize: 12, color: 'var(--blue)' }}>编辑</button>
                <button onClick={() => onChange(models.filter((_, j) => j !== i))} className="opacity-0 group-hover:opacity-100 transition-opacity" style={{ fontSize: 12, color: 'var(--t4)' }}
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button onClick={add} disabled={!newId.trim()} style={{ padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: 'var(--blue)', color: 'white', border: 'none', cursor: 'pointer', opacity: newId.trim() ? 1 : 0.4, whiteSpace: 'nowrap' }}>添加</button>
      </div>
...
export default function SettingsPanel() {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 10, background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--t3)', fontSize: 14, transition: 'all 0.15s', fontFamily: 'var(--font)' }}
    ...
    >
      ...
      <span>AI 设置</span>
      <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--t4)' }}>{currentConfig.label}</span>
    </button>
  )

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(12px)' }}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid var(--sep)' }}>
          <div>
          ...
          <button onClick={() => setOpen(false)}
            style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--bg-3)', border: 'none', cursor: 'pointer', color: 'var(--t3)', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'var(--bg-4)'; (e.currentTarget as HTMLButtonElement).style.color = 'var(--t1)' }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'var(--bg-3)'; (e.currentTarget as HTMLButtonElement).style.color = 'var(--t3)' }}
          >✕</button>
        </div>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 28 }}>
              ...
              {PROVIDER_CONFIGS.map(p => (
                <button key={p.id} onClick={() => { const ms = getProviderModels(p.id, aiSettings.customModels); updateAISettings({ providerId: p.id, modelId: ms[0]?.id ?? '' }); setManagingModels(null) }}
                  style={{
                    ...
                    background: aiSettings.providerId === p.id ? 'var(--blue-bg)' : 'var(--bg-3)',
                    border: `1px solid ${aiSettings.providerId === p.id ? 'var(--blue-border)' : 'var(--b1)'}`,
                    color: aiSettings.providerId === p.id ? 'var(--blue-hover)' : 'var(--t2)',
                  }}
                  onMouseEnter={e => { if (aiSettings.providerId !== p.id) { (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--b2)'; (e.currentTarget as HTMLButtonElement).style.color = 'var(--t1)' } }}
                  onMouseLeave={e => { if (aiSettings.providerId !== p.id) { (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--b1)'; (e.currentTarget as HTMLButtonElement).style.color = 'var(--t2)' } }}
                >
                  {p.label}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <button onClick={() => setManagingModels(managingModels ? null : aiSettings.providerId)} style={{ fontSize: 12, color: 'var(--blue)', background: 'none', border: 'none', cursor: 'pointer' }}>
                ...
                <div key={p.id}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={() => saveKey(p.id)} disabled={!keyDraft[p.id]?.trim()}
                      style={{ padding: '8px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: 'var(--blue)', color: 'white', border: 'none', cursor: 'pointer', opacity: keyDraft[p.id]?.trim() ? 1 : 0.4, whiteSpace: 'nowrap', transition: 'opacity 0.15s' }}>保存</button>
                  </div>
                </div>
              ))}
            </div>
